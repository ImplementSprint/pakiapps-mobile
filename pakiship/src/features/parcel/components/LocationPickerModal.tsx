...
// NCR cities/municipalities — restrict bookings to Metro Manila only
const NCR_KEYWORDS = [
  'manila', 'quezon city', 'caloocan', 'las piñas', 'las pinas',
  'makati', 'malabon', 'mandaluyong', 'marikina', 'muntinlupa',
  'navotas', 'parañaque', 'paranaque', 'pasay', 'pasig', 'pateros',
  'san juan', 'taguig', 'valenzuela', 'metro manila', 'ncr',
  'national capital region',
];

function isWithinNCR(address: string): boolean {
  const normalized = address.toLowerCase();
  return NCR_KEYWORDS.some((keyword) => normalized.includes(keyword));
}
...
export default function LocationPickerModal({ isOpen, onClose, onSelect, type }: Props) {
  ...
  const handleUseCurrentLocation = async () => {
    ...
    try {
      ...
      if (!isWithinNCR(address)) {
        Alert.alert(
          'Outside Service Area',
          'PakiSHIP currently only services Metro Manila (NCR). Please select a location within the National Capital Region.',
          [{ text: 'OK' }],
        );
        return;
      }
