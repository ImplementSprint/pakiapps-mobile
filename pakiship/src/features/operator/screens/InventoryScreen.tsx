...
function StatusBadge({ status, draftStatus }: { status: string; draftStatus?: string | null }) {
  let cfg = (STATUS_CONFIG[status] || STATUS_CONFIG.incoming) as { label: string; color: string; bg: string; icon: string };
  if (status === "dispatched" && draftStatus === "delivered") {
    cfg = { label: "Delivered by Driver", color: COLORS.green, bg: COLORS.greenLight, icon: "check-circle" };
  }
  return (
...
function ParcelCard({ parcel, onReceive, onPickup, onDispatch, processing }: { parcel: HubParcel; onReceive: (p: HubParcel) => void; onPickup: (p: HubParcel) => void; onDispatch: (p: HubParcel) => void; processing?: boolean }) {
  ...
  const isDispatchedDelivered = parcel.status === "dispatched" && parcel.draftStatus === "delivered";
  return (
    <View style={styles.parcelCard}>
      ...
      <View style={styles.parcelFooter}>
        <StatusBadge status={parcel.status} draftStatus={parcel.draftStatus} />
        {isIncoming && (
          <TouchableOpacity
      ...
      {isStored && (
        <View style={styles.storedActions}>
          <TouchableOpacity style={[styles.dispatchBtn, { flex: 1 }]} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); onDispatch(parcel); }} activeOpacity={0.85}>
            <Feather name="send" size={14} color={COLORS.white} />
            <Text style={styles.dispatchBtnText}>Dispatch Driver</Text>
          </TouchableOpacity>
        </View>
      )}
      {isDispatchedDelivered && (
        <View style={styles.storedActions}>
          <TouchableOpacity style={[styles.dispatchBtn, { flex: 1, backgroundColor: COLORS.green }]} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); onPickup(parcel); }} activeOpacity={0.85}>
            <Feather name="check-circle" size={14} color={COLORS.white} />
            <Text style={styles.dispatchBtnText}>Scan Pickup</Text>
          </TouchableOpacity>
        </View>
      )}
    </View>
...
export default function InventoryScreen({
  ...
  async function handleScanConfirmed(scannedData?: string) {
    ...
    try {
      ...
      console.log(`[InventoryScreen] calling dispatchParcel with recordId="${recordId}"`);
      await dispatchParcel(recordId);
      setDispatchParcelItem(scanParcel);
      setShowDispatchSuccess(true);
      loadParcels();
    } catch (e: any) {
      console.error(`[InventoryScreen] receive and dispatch failed:`, e?.message, e);
      Alert.alert("Error", e?.message ?? "Could not receive and dispatch parcel.");
      setScanParcel(null);
    } finally {
      setProcessing(false);
    }
  ...
  const filteredParcels = parcels.filter((p) => {
    ...
    if (filter === "stored") return p.status === "stored" || (p.status === "dispatched" && p.draftStatus === "delivered");
    return true;
  });

  const incomingCount = parcels.filter((p) => p.status === "incoming").length;
  const storedCount = parcels.filter((p) => p.status === "stored" || (p.status === "dispatched" && p.draftStatus === "delivered")).length;
